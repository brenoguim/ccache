               ...
               const util::ProgressReceiver& progress_receiver)
{
  ...
  auto on_disk_size_str = format_human_readable_size(on_disk_size);
  auto cache_size_str = format_human_readable_size(compr_size + incompr_size);
  auto compr_size_str = format_human_readable_size(compr_size);
  auto compr_orig_size_str = format_human_readable_size(compr_orig_size);
  auto incompr_size_str = format_human_readable_size(incompr_size);

  printf("Total data:            %8s (%s disk blocks)\n",
         cache_size_str.get(),
         on_disk_size_str.get());
  printf("Compressible data:     %8s (%.1f%% of original size)\n",
         compr_size_str.get(),
         100.0 - savings);
  printf("  - Original size:     %8s\n", compr_orig_size_str.get());
  printf(
    "  - Compression ratio: %5.3f x  (%.1f%% space savings)\n", ratio, savings);
  printf("Incompressible data:   %8s\n", incompr_size_str.get());
}
